...
(function(){
  ...
  function init(){
    ...
    function card(p){const d=destinations.find(x=>x.slug===p.destination);const image=map[d?.name]||window.GUNINA_TRAVEL_IMAGES.fallback(d?.name||p.destination,d?.region||"");return `<article class="card"><div class="card-media"><img loading="lazy" src="${esc(image)}" alt="${esc(p.title)}" onerror="this.onerror=null;this.src='${esc(window.GUNINA_TRAVEL_IMAGES.fallback(d?.name||p.destination,d?.region||''))}'></div><div class="card-body"><span class="eyebrow">${esc(p.duration)}</span><h3>${esc(p.title)}</h3><p>${esc(p.overview)}</p>${(p.highlights&&p.highlights.length)?`<p class="card-meta"><strong>Highlights:</strong> ${esc(p.highlights.slice(0,4).join(" · "))}</p>`:""}<a class="btn outline" href="packages/${esc(p.slug)}.html">View Package</a></div></article>`;}
